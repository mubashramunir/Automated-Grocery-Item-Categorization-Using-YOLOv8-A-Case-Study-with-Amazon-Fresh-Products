# -*- coding: utf-8 -*-
"""Automated Grocery Item Categorization with Amazon Fresh Products.ipynb
...
model_path = '/content/runs/detect/yolov8_custom_model22'
model = YOLO(model_path)

def predict(image):
    # Convert the image to a numpy array if it's in PIL format
    ...
    # Initialize a list to hold class names
    class_names = []

    # Convert results to image format and extract class names
    ...
    class_summary = ', '.join(set(class_names))  # Use set to avoid duplicate names

    # Return the image and class summary
    return Image.fromarray(cv2.cvtColor(im_array, cv2.COLOR_BGR2RGB)), class_summary
...
interface = gr.Interface(
    ...
    description="Upload an image to detect objects using YOLOv8 and get category names."
)
